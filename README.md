...
| Command                                  | Description                                |
| ---------------------------------------- | ------------------------------------------ |
| `메뉴` / `menu`                          | 카페 메뉴 목록을 조회합니다.               |
| `주문 생성 [메모]` / `order new [메모]`  | 새 주문을 생성합니다.                      |
| `주문 선택 <주문_id>` / `order select <주문_id>` | 기존 주문을 현재 주문으로 선택합니다.      |
| `주문 추가 <메뉴_id> <수량> [옵션]` / `order add <메뉴_id> <수량> [옵션]`     | 현재 주문에 메뉴를 추가합니다.             |
| `주문 삭제 <라인번호>` / `order remove <라인번호>`                  | 현재 주문에서 특정 주문 항목을 삭제합니다. |
| `주문 조회` / `order show`                             | 현재 주문 내용을 조회합니다.               |
| `주문 취소` / `order cancel`                             | 현재 주문을 취소합니다.                    |
| `주문목록 목록 [진행중\|결제완료\|취소]` / `orders list [open\|paid\|canceled]`| 주문 목록을 조회합니다.                    |
| `결제 <방법> [금액]` / `pay <방법> [금액]`                    | 현재 주문을 결제 완료 처리합니다.          |
| `도움말` / `help`                                | 사용 가능한 명령어를 출력합니다.           |
| `종료` / `끝` / `exit` / `quit`                                  | 프로그램을 종료합니다.                     |
